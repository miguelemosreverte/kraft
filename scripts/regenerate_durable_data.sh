#!/bin/bash
# Regenerate Durable Execution demo outputs
# Runs all 11 demos and captures their output to docs/durable/seed_data.sql

set -e

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PROJECT_ROOT="$(dirname "$SCRIPT_DIR")"
OUTPUT_FILE="$PROJECT_ROOT/docs/durable/seed_data.sql"

echo "════════════════════════════════════════════════════════════"
echo "  Regenerating Durable Execution Demo Outputs"
echo "════════════════════════════════════════════════════════════"
echo ""
...
capture_demo 11 "Multi-Node Cluster" "Cluster formation with SWIM gossip" "Demo11"
capture_demo 12 "Distributed Execution" "Workflow routing via consistent hashing" "Demo12"
capture_demo 13 "Failure & Rebalancing" "Node failure detection and recovery" "Demo13"
...
echo "════════════════════════════════════════════════════════════"
echo ""
echo "To regenerate the book HTML, run: make durable-regenerate"
